...
8. `claude plugin install cro@cro-plugin` e conferir `claude plugin details cro@cro-plugin` → `Skills (10)`.
9. Abrir o `claude`, digitar `/` e confirmar que as skills aparecem com prefixo `cro:`.
10. Se o teste foi só de validação, limpar: `claude plugin uninstall cro@cro-plugin` + `claude plugin marketplace remove cro-plugin`.
